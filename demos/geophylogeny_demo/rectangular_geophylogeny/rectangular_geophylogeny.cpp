...
int main(int argc, char* argv[]) {
	...
	*/
	...
	for (auto& node: tree->m_inner_nodes) {
	...
	DPOrdenerRectangular ordener = DPOrdenerRectangular(geophy1, DPOrdenerRectangular::DPStrategy::kEuclidean);
	ordener.orderLeaves();

	geophy1->setInnerPositions(geophy1->m_tree->m_root);

	//geophy1->m_tree->setColors();

	//std::cout << geophy1->m_tree->depthOfTree(geophy1->m_tree->m_root) << std::endl;

	/*for (auto& site: geophy1->m_sites) {
		std::cout << site->m_interval.from() <<" ";
		std::cout << site->m_position.x() << " ";
		std::cout << site->m_interval.to() << std::endl;
	}
	...
	painting = std::make_shared<Painting>(geophy1);
